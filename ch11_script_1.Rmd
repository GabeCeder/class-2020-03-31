...
a <- county %>% 
  mutate(combined = less_than_hs + hs)
...
```{r}
county %>% 
  summarize(pov_no_hs = cor(less_than_hs, poverty))
...
```{r}
ggplot(a, aes(x = less_than_hs, y = poverty)) +
  geom_point() +
  geom_abline()
```
...
ggplot(a, aes(x = less_than_hs, y = poverty, color = region)) +
  geom_point() +
  gganimate()
